...
void PlayMapMgt::inital(CDirectX * directX, int screenWidth, int screenHeight){
	...
	this->_winner_background = LoadSurfaceFromFile(directX->getDevice(), WINNER_MAP_IMG, NULL);
}
...
void PlayMapMgt::gameWinner(){
	this->_is_winner = true;
	this->_time_waiting_winner = GetTickCount();
	this->_play_sound_bg = false;
	this->_is_game_over = false;
	SoundManager::GetInstance()->getSoundWithID(ID_SOUND_BACKGROUND)->Stop();
...
void PlayMapMgt::update(int deltaTime, CDirectX * directX){
	...
	if (!this->_is_game_over && this->_b_map_times < 1 && !this->_is_winner)
	{
		this->gameOver();
	...
	if (this->_is_winner && (now - this->_time_waiting_winner) > MAP_TIME_WAITING){
	...
	if (this->_is_game_over || this->_is_game_starting || this->_is_winner)
		return;
			...
			if (dir != DIRECTION::NONE){
				if (((DoorObject*)this->_list_map.at(this->_current_map)->_list_objects.at(i))->isOpen()){
					...
					case DIRECTION::BOTTOM:
						SoundManager::GetInstance()->getSoundWithID(ID_SOUND_WINNER)->Play();
						Sleep(1000);
						if (this->_current_map >= this->_list_map.size() - 1)
						{
							gameWinner();
							break;
						}
...
void PlayMapMgt::draw(CDirectX * directX){
	...
	if (this->_is_winner){
		this->_gb_text_mgt->Initialize(directX->getDevice(), TEXT("Kristen ITC"), 18, 1);
		directX->getDevice()->StretchRect(this->_winner_background, NULL, directX->getBackBuffer(), NULL, D3DTEXF_NONE);
		std::string mapStr = std::to_string(this->_b_gold);
		char *cstr = new char[mapStr.length() + 1];
		strcpy(cstr, mapStr.c_str());
		this->_gb_text_mgt->Print(cstr, 288, 332, D3DCOLOR_XRGB(206, 230, 5), directX->getSpriteHandler(), 0, 0, FA_LEFT);
		delete[] cstr;
		return;
	}

	if (this->_is_game_starting){
		this->_gb_text_mgt->Initialize(directX->getDevice(), TEXT("Time New Roman"), 35, 1);
		...
		char *cstr = new char[mapStr.length() + 1];
		strcpy(cstr, mapStr.c_str());
		this->_gb_text_mgt->Print(cstr, 288, 220, D3DCOLOR_XRGB(255, 0, 0), directX->getSpriteHandler(), 0, 0, FA_LEFT);
	
		if (this->_list_map.size() <= this->_current_map){
			return;
		}
		this->_gb_text_mgt->Initialize(directX->getDevice(), TEXT("Kristen ITC"), 18, 1);
		std::string mapKeys = std::to_string(this->_list_map.at(this->_current_map)->getNeedKeys()) + "";
		cstr = new char[mapKeys.length() + 1];
		strcpy(cstr, mapKeys.c_str());
		this->_gb_text_mgt->Print(cstr, 247, 331, D3DCOLOR_XRGB(206, 230, 5), directX->getSpriteHandler(), 0, 0, FA_LEFT);
		delete[] cstr;
		return;
	}
